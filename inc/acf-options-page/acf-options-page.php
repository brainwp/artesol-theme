...

{
	...
	*/
	function __construct()
	{
		...
		add_filter('acf/location/rule_values/options_page', array(
			$this,
	...
	*/
	function admin_menu()
	{
		...
		$parent_page = add_menu_page($parent_title, $parent_menu, $this->settings['capability'], $parent_slug, array(
			$this,
			'html'
		));
		...
		add_action('load-' . $parent_page, array(
			$this,
			'admin_load'
		));
		if (!empty($this->settings['pages'])) {
			foreach($this->settings['pages'] as $c) {
				...
				$child_page = add_submenu_page($parent_slug, $sub_title, $sub_title, $this->settings['capability'], $sub_slug, array(
					$this,
					'html'
				));
				...
				add_action('load-' . $child_page, array(
					$this,
					'admin_load'
				));
			}
	...
	function admin_head()
	{
		...
		if ($acfs) {
			foreach($acfs as $acf) {
				...
				add_meta_box('acf_' . $acf['id'], $acf['title'], array(
					$this,
					'meta_box_input'
				) , 'acf_options_page', $acf['options']['position'], 'high', array(
					'field_group' => $acf,
					'show' => $show,
					'post_id' => 'options'
				));
...
?>
		...
		<div class="wrap no_move">
			...
			<h2><?php
		echo get_admin_page_title(); ?></h2>



			<?php
		if (isset($this->data['admin_message'])): ?>

			<div id="message" class="updated"><p><?php
			echo $this->data['admin_message']; ?></p></div>
			...
			<?php
		if (isset($this->data['no_fields'])): ?>

			<div id="message" class="updated"><p><?php
			_e("No Custom Field Group found for the options page", 'acf'); ?>. <a href="<?php
			echo admin_url(); ?>post-new.php?post_type=acf"><?php
			_e("Create a Custom Field Group", 'acf'); ?></a></p></div>
			...
			echo $_SERVER['HTTP_REFERER'] ?>" />
							...
							<input type="hidden" name="acf_nonce" value="<?php
			echo wp_create_nonce('input'); ?>" />

							<input type="submit" class="acf-button" value="<?php
			_e("Save Options", 'acf'); ?>" />
					...
					<?php
			$meta_boxes = do_meta_boxes('acf_options_page', 'side', null); ?>
				...
				<div id="post-body-content">
					...
					<?php
			$meta_boxes = do_meta_boxes('acf_options_page', 'normal', null); ?>
